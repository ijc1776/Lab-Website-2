...
function loadStatsPage() {
	...
	for (row_counter = 2; row_counter < table.rows.length; row_counter++) {
		a = parseInt(table.rows[row_counter].cells[2].innerHTML);
		b = parseInt(table.rows[row_counter].cells[3].innerHTML);
		if (a > b) {
	...
	var win_stat = document.getElementById("wins");
	var loss_stat = document.getElementById("losses");

	win_stat.innerHTML = win;
	loss_stat.innerHTML = loss;
}
...
function loadPlayersPage() {
	/*var index = 0;
	var myElement = document.getElementById("player_selector");
	for (index = 0; index < players.length; index += 1) {
		const tag = document.createElement('a');
		tag.setAttribute('href', '#');
		tag.id = index;
		tag.innerText = players[index].name + '\n';
	...
	a1.setAttribute("onclick", "switchPlayers(0)");
	a2.setAttribute("onclick", "switchPlayers(1)");
	a3.setAttribute("onclick", "switchPlayers(2)");
	a4.setAttribute("onclick", "switchPlayers(3)");

	a1.innerHTML = players[0].name + '\n';
	a2.innerHTML = players[1].name + '\n';
	a3.innerHTML = players[2].name + '\n';
	a4.innerHTML = players[3].name + '\n';
	
	linebreak = document.createElement('br');
	
	ps.appendChild(a1);
	ps.appendChild(linebreak);
	ps.appendChild(a2);
	ps.appendChild(linebreak);
	ps.appendChild(a3);
	ps.appendChild(linebreak);
...
function switchPlayers(playerNum) {
	...
	var avgPassingYards = p_yards.innerHTML / played.innerHTML;
	avg_p_yards.innerHTML = Math.round(avgPassingYards);

	var avgRushYards = r_yards.innerHTML / played.innerHTML;
	avg_r_yards.innerHTML = Math.round(avgRushYards);

	var avgRecievingYards = rec_yards.innerHTML / played.innerHTML;
	avg_rec_yards.innerHTML = Math.round(avgRecievingYards);
}
